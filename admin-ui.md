...
Central Application routes are prefixed with `/central`. Tenant auth uses root paths (`/login`, `/register`, …). See [authentication/authentication-developer.md](authentication/authentication-developer.md).
...
| Payment Gateways | `/payment-gateways`, enable/disable/default/config/mode/test-connection/logs |
