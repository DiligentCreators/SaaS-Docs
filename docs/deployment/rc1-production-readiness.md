...
- [ ] `FILESYSTEM_DISK=s3` + `AWS_*` configured; `storage:migrate-to-s3` if migrating local branding; bucket backups ([object-storage.md](/developer-guide/object-storage))
...
- [architecture/platform-production-runbook.md](/deployment/platform-production-runbook)
- [billing/payment-gateways-production.md](/deployment/payment-gateways)
- [authentication/authentication-production.md](/deployment/authentication)
